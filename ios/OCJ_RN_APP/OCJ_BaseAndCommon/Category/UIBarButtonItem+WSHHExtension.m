//
//  UIBarButtonItem+WSHHExtension.m
//  OCJ
//
...
#import "UIBarButtonItem+WSHHExtension.h"
#import <objc/runtime.h>

@implementation UIBarButtonItem (WSHHExtension)
...
- (void)badgeInit
{
    ...
    self.badgePadding   = 6;
    self.badgeMinSize   = 3;
    self.badgeOriginX   = self.customView.frame.size.width - self.badge.frame.size.width/2-7;
    self.badgeOriginY   = 4;
...
- (CGSize) badgeExpectedSize
{
    ...
    OCJBaseLabel *frameLabel = [self duplicateLabel:self.badge];
...
- (OCJBaseLabel *)duplicateLabel:(OCJBaseLabel *)labelToCopy
{
    OCJBaseLabel *duplicateLabel = [[OCJBaseLabel alloc] initWithFrame:labelToCopy.frame];
...
-(void)setBadge:(OCJBaseLabel *)badgeLabel
{
...
-(void) setBadgeValue:(NSString *)badgeValue
{
    ...
    if (!badgeValue || ([badgeValue isEqualToString:@"0"] && self.shouldHideBadgeAtZero)) {
        [self removeBadge];
    } else if (!self.badge) {
        // Create a new badge because not existing
        self.badge                      = [[OCJBaseLabel alloc] initWithFrame:CGRectMake(self.badgeOriginX, self.badgeOriginY, 2, 2)];
        self.badge.textColor            = self.badgeTextColor;
        self.badge.backgroundColor      = self.badgeBGColor;
        self.badge.font                 = self.badgeFont;
