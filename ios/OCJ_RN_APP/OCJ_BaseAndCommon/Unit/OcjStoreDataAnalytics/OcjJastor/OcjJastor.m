...
- (id)initWithDictionary:(NSDictionary *)dictionary {
	...
	if ((self = [super init])) {
		for (NSString *key in [OcjJastorRuntimeHelper propertyNames:[self class]]) {
            ...
            if ([OcjJastorRuntimeHelper isPropertyReadOnly:[self class] propertyName:key]) {
                continue;
            }
			
			// handle dictionary
			if ([value isKindOfClass:nsDictionaryClass]) {
				Class klass = [OcjJastorRuntimeHelper propertyClassForPropertyName:key ofClass:[self class]];
			...
			// handle array
				...
				for (id child in value) {
                    if ([[child class] isSubclassOfClass:nsDictionaryClass]) {
                        ...
                        } else if ([arrayItemType isSubclassOfClass:[OcjJastor class]]) {
                            OcjJastor *childDTO = [[arrayItemType alloc] initWithDictionary:child];
...
- (NSMutableDictionary *)toDictionary {
	...
	for (NSString *key in [OcjJastorRuntimeHelper propertyNames:[self class]]) {
        ...
        if (value && [value isKindOfClass:[OcjJastor class]]) {
            ...
            if (internalValue && [internalValue isKindOfClass:[OcjJastor class]]) {
...
- (NSDictionary *)map {
	NSArray *properties = [OcjJastorRuntimeHelper propertyNames:[self class]];
...
- (BOOL)isEqual:(id)object {
	if (object == nil || ![object isKindOfClass:[OcjJastor class]]) return NO;
	
	OcjJastor *model = (OcjJastor *)object;
