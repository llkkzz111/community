//
//  OCJGlobalScreenTransverseCollectionCell.m
//  OCJ
//
...
@interface OCJGlobalScreenTransverseCollectionCell()
...
@property (nonatomic,strong) UILabel        *ocjLab_Notedesc;
...
@implementation OCJGlobalScreenTransverseCollectionCell
...
- (void)ocj_addViews
{
    ...
    [self.ocjLab_title mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(self.ocjImageView_icon.mas_right).offset(10);
        make.size.mas_equalTo(CGSizeMake(SCREEN_WIDTH - 156.5, 42));
    }];
  ...
  [topLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    make.left.mas_equalTo(self.ocjImageView_icon.mas_right).offset(10);
    make.top.mas_equalTo(self.ocjImageView_icon.mas_top).offset(2);
    ...
    [self.ocjLab_Note mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.ocjLab_title.mas_bottom).offset(5.5);
        make.left.mas_equalTo(self.ocjImageView_icon.mas_right).offset(10);
        make.size.mas_equalTo(CGSizeMake(28, 15));
    }];
    
    [self addSubview:self.ocjLab_Notedesc];
    [self.ocjLab_Notedesc mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.ocjLab_title.mas_bottom).offset(5.5);
        make.left.mas_equalTo(self.ocjLab_Note.mas_right).offset(5);
    ...
    [self.ocjLab_Price mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.ocjLab_Note.mas_bottom).offset(35);
        make.left.mas_equalTo(self.ocjImageView_icon.mas_right).offset(10);
    }];
...
{
  __weak OCJGlobalScreenTransverseCollectionCell *weakSelf = self;
  ...
  [self.ocjLab_warning setText:stockStatus];
   ...
   if (gift.length>0) {
      ...
      [self.ocjLab_Notedesc setText:gift];
   }else{
     
      [self.ocjLab_Note setText:@""];
      [self.ocjLab_Notedesc setText:@""];
   }
...
- (UILabel *)ocjLab_Notedesc
{
    if (!_ocjLab_Notedesc) {
        _ocjLab_Notedesc =[[UILabel alloc] initWithFrame:CGRectZero];
        [_ocjLab_Notedesc setBackgroundColor:[UIColor clearColor]];
        _ocjLab_Notedesc.font = [UIFont systemFontOfSize:11];
        _ocjLab_Notedesc.textColor =  [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1/1.0];
    }
    return _ocjLab_Notedesc;
}
