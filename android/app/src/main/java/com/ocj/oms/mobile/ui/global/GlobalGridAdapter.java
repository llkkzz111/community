...
public class GlobalGridAdapter extends BaseQuickAdapter<CmsItemsBean, BaseViewHolder> {

    Context ctx;

    public GlobalGridAdapter(Context ctx, @LayoutRes int layoutResId, @Nullable List<CmsItemsBean> data) {
    ...
    protected void convert(BaseViewHolder holder, CmsItemsBean data) {
        ...
        if (isNotNull(data.getInStock()) && !NumUtils.checkStringIsNum(data.getInStock())) {
            ...
            holder.getView(R.id.tv_storage).setVisibility(View.INVISIBLE);
        }
