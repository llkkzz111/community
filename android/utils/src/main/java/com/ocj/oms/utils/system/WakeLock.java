package com.ocj.oms.utils.system;
...
public class WakeLock {
    ...
    public boolean isScreenOn() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ECLAIR_MR1) {
            Logger.e("can not call isScreenOn if SDK_INT < 7 ");
            return false;
        } else {
    ...
    public void turnScreenOn() {
        //点亮亮屏
        Logger.i("PowerManager.WakeLock : wakeLock.isHeld: " + wakeLock.isHeld());
        if (!wakeLock.isHeld()) {
            Logger.i("PowerManager.WakeLock : 点亮屏幕");
            wakeLock.acquire();
        }
    }
    ...
    public void turnScreenOff() {
        ...
        Logger.i("PowerManager.WakeLock : wakeLock.isHeld: " + wakeLock.isHeld());
        if (wakeLock.isHeld()) {
            Logger.i("PowerManager.WakeLock : 灭掉屏幕");
            try {
                wakeLock.release();
            } catch (Exception e) {
                e.printStackTrace();
