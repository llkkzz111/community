        ...
        switch (messageType) {
            case SEND_MESSAGE:
                ...
                if (baseResponse.getCode() != null && baseResponse.getCode().equals(ResponseCode.CHECK_FRIENDSHIP_FAIL)) {
                    sendLocalTextMsg("很抱歉，你们还不是好友关系，请先发送好友验证请求，对方验证通过后，才能聊天。<发送好友验证|choudao://friend/verification?id=" + nowChatId + "&msg=我是" + DBHelper.getInstance().queryMyInfo().getName() + " />");
                }
                break;
        }
    }
