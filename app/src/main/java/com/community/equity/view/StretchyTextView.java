...
/**
 * Created by liuzhao on 16/12/27.
 */
    ...
    class InnerRunnable implements Runnable {
        ...
        public void run() {
            if (mState == SPREADTEXT_STATE_SPREAD) {
                Logger.e(contentText.getLineCount() + "-----mState == SPREADTEXT_STATE_SPREAD------" + maxLineCount + (contentText.getLineCount() > maxLineCount));
                if (contentText.getLineCount() > maxLineCount) {
                    operateText.setVisibility(View.VISIBLE);
                } else if (contentText.getLineCount() == maxLineCount) {
                } else {
                    operateText.setVisibility(View.GONE);
                }
