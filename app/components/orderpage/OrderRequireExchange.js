...
import GetChangeListRequest from '../../../foundation/net/mine/GetChangeListRequest';
import NetErro from '../error/NetErro';
import {Actions} from 'react-native-router-flux';
import OrderCenterItem from './OrderCenterItem';
var key = 0;
export default class OrderRequireExchange extends Component {
    constructor(props) {
        super(props);
        this.state = {
            pageNum: 1,
        ...
        this._showFapiao = this._showFapiao.bind(this);
    }
    ...
    _renderItem({item, index}) {
        ...
        let state = '';
        let itemOne = {};
        let isShowFapiao = true;
        let isShowExchange = true;
        let isShowWuliu = true;
    ...
    _doPost(showLoading = true) {
        let self = this;
        if (this.getChangeListRequest) this.getChangeListRequest.setCancled(true);
        let param = {};
        this.getChangeListRequest = new GetChangeListRequest(param, 'GET');
        if (showLoading) this.getChangeListRequest.showLoadingView();
        this.getChangeListRequest.start((response) => {
            this.setState({
                isShowEmpty: true,
                isRefresh: false,
            });
     ...
     */
    _showFapiao(item) {
        Actions.BillListDetailPage({order_no: item.order_no, order_g_seq: '001'});
    }
    ...
    _viewExchangeDetail(item) {
        ...
        if (item.items && item.items.length > 0) {
            orderSeq = item.items[0].order_g_seq;
        }
